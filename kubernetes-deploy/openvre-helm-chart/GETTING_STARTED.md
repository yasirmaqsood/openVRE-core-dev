...
**Branch:** `kubernetes-interactive-pod-with-auth` — interactive pods plus OpenVRE login on session URLs (gateway + ingress auth).
...
After base install, see **[../INTERACTIVE.md](../INTERACTIVE.md)** for auth URLs and `interactive.openvreAuth` settings.
---
...
## 13. Interactive pods with OpenVRE auth (this branch only)

Complete **Sections 2–9** first (same cluster prerequisites as the base chart).
...
2. Build **frontend** from this branch (includes `applib/interactiveAuth.php`, `interactiveGateway.php`, `interactiveLoginStart.php`).
3. Build **scheduler** from `../scheduler/` — must receive `OPENVRE_INTERACTIVE_AUTH_URL` / `SIGNIN` when auth is enabled.
4. Follow **[../INTERACTIVE.md](../INTERACTIVE.md)** for Mongo tool and user flow.

**Additional prerequisites for auth:**

| Item | Detail |
|------|--------|
| Ingress can reach frontend Service | `auth-url` subrequest hits `interactiveAuth.php` in-cluster |
| Public `signInUrl` | Browser redirect for users not logged in |
| Gateway reachable | UI links use `/applib/interactiveGateway.php?path=...` |

Compare with pods-only branch `kubernetes-interactive-pod` (Section 13 there has no auth).
